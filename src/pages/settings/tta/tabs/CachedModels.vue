          ...
          <model-list
            :models="
              settingStore.availableModels.map((modelName: string) => ({
                ...
                disabled: findSettingByKey(settingStore.ttaSettings, TextToActionSettingKey.AutoCachedModels)?.value.find((m: Record<string, any>) => m.name === modelName),
              }))
            "
            @on-model-click="(model: any) => cacheModel(model.name)"
          />
        </v-menu>
